        ...
        const fetchData = async () => {
            try {
                ...
                await axios
                    ...
                    .then((response) => {
                        localStorage.setItem('IDUser', JSON.stringify(response.data.data.data.ID));
                        try {
                            const token = localStorage.getItem('token-login');
                            const _token = token.substring(1, token.length - 1);
                            axios
                                .post(
                                    'http://localhost:8080/history/ticket/' +
                                ...
                                .then((response) => {
                                    setListTicket(response.data.result);
                                });
                            axios
                                .post(
                                    'http://localhost:8080/history/food/' + JSON.parse(localStorage.getItem('IDUser')),
                                    { x: 1 },
                                    {
                                        headers: {
                                            'content-type': 'application/x-www-form-urlencoded',
                                ...
                                .then((response) => {
                                    setListFood(response.data.result);
                                });
